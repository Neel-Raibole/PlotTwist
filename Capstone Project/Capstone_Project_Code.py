...
def preprocess_pie_chart(full_image_path, plotting_area_coords):
    ...
    Steps:
      ...
      2. Converts the cropped image to Lab color space and extracts the L channel.
      3. Applies CLAHE on the L channel for local contrast enhancement.
      4. Applies a light Gaussian blur to the enhanced L channel to help preserve smooth edges.
      5. Converts the cropped image to HSV and applies enhancement on the S and V channels.
    ...
    Returns:
        tuple: (enhanced_l, cropped_hsv_enhanced)
            - enhanced_l: Enhanced grayscale image (from the L channel) for segmentation.
    ...
    # Convert cropped image to HSV color space and process S and V channels
    hsv = cv2.cvtColor(cropped, cv2.COLOR_BGR2HSV)
    h, s, v = cv2.split(hsv)
    
    # Apply median filtering to the H channel
    ...
    # Apply CLAHE to S and V channels
    clahe_sv = cv2.createCLAHE(clipLimit = 1.5, tileGridSize = (4, 4))
    s = clahe_sv.apply(s)
    v = clahe_sv.apply(v)
    
    # Merge enhanced H, S, V back into one HSV image
    cropped_hsv = cv2.merge([h, s, v])
    
    return cropped_gray, cropped_hsv
...
def extract_legend_color_blocks(image_path, legend_coordinates, min_size_ratio = 0.02, max_size_ratio = 0.5, aspect_ratio_range = (0.5, 4.0)):
    """
...
def process_legend(image_path, legend_coordinates):
    ...
    # Apply Gaussian Blur to the S channel
    s_channel = hsv_img[:, :, 1]
    ...
    # display(Image.fromarray(s_thresh))  # Uncomment for visualization
...
def process_pie_chart_and_map_legend(image_path, legend_coordinates, pie_segments):
    ...
    def extract_legend_info(legend_img):
        ...
        for cnt in contours:
            ...
            print("legend_w legend_h", legend_w, legend_h)
            if w < legend_w * 0.02 or h < legend_h * 0.02 or w > legend_w * 0.5 or h > legend_h * 0.5:
                continue
            # Filter based on aspect ratio
            aspect_ratio = w / float(h)
